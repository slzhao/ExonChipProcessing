...
awk '$0!~"#" {print $1":"$2"\t"$3"\t"$4"\t"$5}' $g1k_dir/1000G.vcf > g1k/1000g_1-22_chr_pos
...
do
    ...
    for race in $race_list
    do
	join -j 1 <(sort -k1,1 g1k/1000g_1-22_chr_pos) <(sort -k1,1 exome/${cohort}_${race}_pos) | awk '{print $1,$3,$4,$8,$9,$10,$12}' OFS="\t" > exome/${cohort}_${race}_ref
...
do
    ...
    echo -e $title > exome/exm_${race}_uniq
	...
	join -j 1 <(sort -k1,1 exome/temp) <(awk '$0!~"POS" {print $1,$3,$4,$5}' exome/${cohort}_${race}_af | sort -k1,1)  > exome/temp1
	cp exome/temp1 exome/temp
    done
    sed 's/ /\t/g' exome/temp > exome/exm_${race}
    sort -k1,1 -k3,3r -k4,4r exome/exm_${race}| awk '!x[$1]++ {print}' OFS="\t">> final_exm_${race}
done
rm exome/temp
rm exome/temp1

## final output files are final_exm_${race}
...
for race in $race_list
do
        awk '$0!~"POS" {sum=0;fac=0; for (i=5;i<=NF;i+=3) {sum+=$i;fac+=$(i-1)*$i}; print $1,$2,fac,fac/sum,sum}' OFS="\t" final_exm_${race} > exome/exm_${race}_sum
done

# merge g1k and exm
for race in W B H
do
    join -j 1 <(sort -k1,1 ${race}_af_1kg) <(sort -k1,1 exome/exm_${race}_sum) | awk '{if ($3~"0" && $6!~"0") $3=$6; print $1,$2,$3,$4,$5,$7,$8}' | sed 's/ /\t/g' > final_${race}_g1k_exm
    sed -i '1iPOS\tREF/ALT\tA1/A2\tAF\tNCHROBS\texm_AF\texm_NCHROBS' final_${race}_g1k_exm
done

## Final output files are final_${race}_g1k_exm
############## The End ############